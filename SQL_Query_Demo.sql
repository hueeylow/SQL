...
-- Query 4 Statement: Show list of employees ID, annual based salary, new based salary based on percent increment hike, job title and department, whose employees’ job level is 5.
SELECT EmployeeNumber, MonthlyIncome*12 as AnnualBaseSalary, (MonthlyIncome*(PercentSalaryHike/100))+MonthlyIncome as NewAnnualBasedSalary, JobRole, Department
FROM HRDataDB.dbo.HRData as a
	FULL JOIN HRDataDB.dbo.EmpSalary as b
		ON a.EmployeeNumber = b.EmpID
WHERE JobLevel IN(5)
...
-- Query 7 Statement: Show top 5 Employee ID by monthly income, job role, department where monthly income is less than 5000 and training times last year is at least 6 years
