...
SELECT EmployeeNumber, MonthlyIncome*12 as AnnualBaseSalary, 
(MonthlyIncome*(1+(PercentSalaryHike/100)))*12 as NewAnnualBaseSalary, PercentSalaryHike, JobRole, Department
FROM HRDataDB.dbo.HRData as a
	FULL JOIN HRDataDB.dbo.EmpSalary as b
		ON a.EmployeeNumber = b.EmpID
WHERE JobLevel IN(5)
ORDER BY NewAnnualBaseSalary ASC
